...
def clean_and_combine_dataframes(df_bwin, df_betfair):
    ...
    df_surebet_bwin_betfair = pd.merge(
        df_bwin, df_betfair, left_on='Teams_matched_betfair', right_on='Teams')
    df_surebet_bwin_betfair = df_surebet_bwin_betfair[df_surebet_bwin_betfair['Score_betfair'] > 65]
    df_surebet_bwin_betfair = df_surebet_bwin_betfair[[
        'Teams_x', 'btts_x', 'Teams_y', 'btts_y']]
    return df_surebet_bwin_betfair


def find_surebet(frame):
    frame[['btts_x_1', 'btts_x_2']] = frame['btts_x'].apply(
        lambda x: x.split('\n')).apply(pd.Series).astype(float)
    frame[['btts_y_1', 'btts_y_2']] = frame['btts_y'].apply(
    ...
    frame = frame[['Teams_x', 'btts_x', 'Teams_y',
                   'btts_y', 'sure_btts1', 'sure_btts2']]
    ...
    for frame in surebet:
        if len(surebet[frame]) >= 1:
            for i, value in enumerate(surebet[frame]['sure_btts1']):
                ...
                if value < 1:
                    odds1 = float(
                        surebet[frame].at[i, 'btts_x'].split('\n')[0])
                    odds2 = float(
                        surebet[frame].at[i, 'btts_y'].split('\n')[1])
